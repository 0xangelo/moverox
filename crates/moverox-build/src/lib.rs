...
impl<'a> Builder<'a> {
    ...
    /// Combines [`Builder::read_modules`] and [`Builder::write_oxidized_datatypes`].
    pub fn build(self) -> Result<()> {
        let modules = self.read_modules()?;
        self.write_oxidized_datatypes(&modules)
    }

    /// Find and parse the package's Move modules.
    ///
    /// Options having an effect here:
    /// - [`Builder::emit_rerun_if_changed`]
    /// - [`Builder::with_implicit_iota_imports`]
    /// - [`Builder::with_implicit_sui_imports`]
    pub fn read_modules(&self) -> Result<Vec<move_syn::Module>> {
        let move_files = self.collect_move_files()?;
        self.parse_files(&move_files)
    }

    /// Write the oxidized Move datatypes to the [`Builder::out_dir`].
    ///
    /// Options having an effect here:
    /// - [`Builder::moverox_path`]
    /// - [`Builder::map_address`]
    /// - [`Builder::published_at`]
    /// - [`Builder::out_dir`]
    pub fn write_oxidized_datatypes(&self, modules: &[move_syn::Module]) -> Result<()> {
